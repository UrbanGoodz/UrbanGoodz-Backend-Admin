    ...
    public function up(): void
    {
        if (!Schema::hasTable('urban_goodz_business_portal_audit_logs')) {
            Schema::create('urban_goodz_business_portal_audit_logs', function (Blueprint $table) {
                ...
                $table->foreignId('business_client_user_id')->nullable();
                $table->foreign('business_client_user_id', 'ug_bp_audit_user_fk')
                    ->references('id')
                    ->on('urban_goodz_business_client_users');
        ...
        } elseif (!Schema::hasIndex('urban_goodz_business_portal_audit_logs', 'ug_bp_audit_user_fk')) {
            Schema::table('urban_goodz_business_portal_audit_logs', function (Blueprint $table) {
                $table->foreign('business_client_user_id', 'ug_bp_audit_user_fk')
                    ->references('id')
                    ->on('urban_goodz_business_client_users');
            });
