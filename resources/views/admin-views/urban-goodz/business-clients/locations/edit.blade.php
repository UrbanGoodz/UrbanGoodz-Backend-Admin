...
@section('title', $client->company_name . ' - ' . translate('Edit Location'))

@section('content')
    <div class="content container-fluid">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
            <h1 class="page-header-title">{{ translate('Edit Location') }} — {{ $location->name }}</h1>
        ...
        <form action="{{ route('admin.urban-goodz.business-clients.locations.update', [$client->id, $location->id]) }}" method="POST">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="latitude" class="form-control" step="any" value="{{ old('latitude', $location->latitude) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">{{ translate('Longitude') }}</label>
                            <input type="text" name="longitude" class="form-control" step="any" value="{{ old('longitude', $location->longitude) }}">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="email" name="contact_email" class="form-control" value="{{ old('contact_email', $location->contact_email) }}">
                        </div>
                        <div class="col-12">
                            <label class="form-label">{{ translate('Operating Hours') }}</label>
                            <textarea name="operating_hours" class="form-control" rows="2">{{ old('operating_hours', $location->operating_hours) }}</textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">{{ translate('Pickup Instructions') }}</label>
                            <textarea name="pickup_instructions" class="form-control" rows="2">{{ old('pickup_instructions', $location->pickup_instructions) }}</textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">{{ translate('Delivery Instructions') }}</label>
                            <textarea name="delivery_instructions" class="form-control" rows="2">{{ old('delivery_instructions', $location->delivery_instructions) }}</textarea>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6 d-flex align-items-end">
                            <div class="form-check">
                                <input type="checkbox" name="is_active" class="form-check-input" id="isActive" value="1" {{ $location->is_active ? 'checked' : '' }}>
