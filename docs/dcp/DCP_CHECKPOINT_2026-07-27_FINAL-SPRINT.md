...
- Current pushed/deployed SHA before the pending local integration tranche: `bee9c71bdcac9100dc795ca066fda7e6aa369686`
...
- Pending local provider work adds allowlisted `openai`, `openrouter`, `gemini`, and `disabled` providers, fail-closed selection, bounded retries, sanitized health, and deterministic fallback.
- Provider tests: 10 passed, 43 assertions; wider affected tests: 31 passed, 159 assertions.
- A valid provider credential with billing/model access is still required for a successful live health check.
...
- Commit/push/deployment: `bee9c71bdcac9100dc795ca066fda7e6aa369686`.
...
- Pending local work provides a read-only Vendors & Businesses directory and truthful Command Center labels without altering records.
...
- Pending backend work supports `BROADCAST_CONNECTION` with legacy `BROADCAST_DRIVER` fallback, HTTPS Pusher configuration, private account/assignment channels, role-specific authorization endpoints, sanitized events, and Echo Pusher/Reverb selection.
...
## Integration commits pending push/deployment
...
- These commits have passed the complete backend suite but must be pushed before deployment.
