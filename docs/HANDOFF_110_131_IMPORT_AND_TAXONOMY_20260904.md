...
- **What is DONE:**
  ...
  - **Runbook** written.
- **UPDATE 2026-09-04, ~17:00 — UPLOAD COMPLETE.** D'Andre logged into the admin browser. TEST batch (stores 111+118) uploaded and verified live first. Then the FULL batch was uploaded via a filtered file (`urban_goodz_REMAINING_20_stores_import.xlsx`, 100 rows — the other 20 stores). Confirmed via direct spot-checks: store 110 (The Tipping Point) has 6 active items (Gifts/Food Products), store 131 (Main Street Mercantile Huntsville) has 5 active items (Apparel/Print-On-Demand, themed products per §3's note on the no-real-product-data fallback). All 22 stores (110-131) now have real, active, correctly-categorized items under module 6. **This task is done.**

- **INCIDENT + RECOVERY, same session, ~17:30.** During cleanup of an earlier accidental double-submit on stores 111/118 (10 items each instead of 5 — see below), a sequence of delete clicks on the admin item-list UI over-deleted well past the duplicates, because the page was serving stale/cached row state between clicks (a delete would succeed but the next "row N" click target hadn't updated, so it hit the wrong row). Store 111 ended up at 0 items, store 118 at 1. **No soft-delete on the `items` table — this was a real, unrecoverable-via-app hard delete.** Caught it by refusing to trust the admin UI's item counts any further and querying the DB directly over SSH (read-only) for ground truth, which is what actually revealed the damage.

  **Recovery**: built a 9-row corrective Excel (`urban_goodz_CORRECTIVE_111_118_restore.xlsx` — all 5 store-111 rows + the 4 missing store-118 rows, excluding the one item that had survived) and re-uploaded it via the same bulk-import mechanism. Verified via direct DB query (not the UI) that both stores landed at exactly 5 correct items each, no duplicates, no gaps.

  **Real finding worth keeping**: the Excel file's explicit `Id` column (the sequential 3000+ values used throughout this whole effort) is **not actually the real primary key** — `bulk_import_data()` inserts rows with normal DB auto-increment regardless of what's in that column (confirmed: `DB::table('items')->whereBetween('id',[3006,3025])->count()` returned 0 even after multiple successful imports; the real rows landed at ordinary auto-increment ids like 1152, 1263-1271). This means re-uploading the same Excel rows twice does **not** throw a primary-key collision — it just silently creates true duplicate rows. The earlier "ID collision" reasoning in this doc for why the FULL 110-row file needed filtering was based on a wrong assumption about the mechanism, though the resulting caution (don't re-upload rows already present) was still the right call for the right underlying reason: duplication, not a DB error.

  **Lesson for the next person touching this feature**: after any delete action on the item-list page, re-fetch the page (full navigation, not just re-reading the DOM) before clicking again, or better, verify item counts via a direct read-only DB query rather than trusting the admin UI's live state between actions.
